...
function App(props) {
  ...
  const detail= useSelector((state) => state.parts.detail);
  ...
  const onDetailClick = (contactid) => {
    ...
    dispatch(slice.details(contactid));
  };
  const handleNext = (e) => {
    let start2 = start + limit;
    if (start2 > total - limit) start2 = total - limit; //total >limit
  ...
  const opendlgfolder = (contactid) => {
    ...
    dispatch(slice.forlder(contactid));
  };
  const opendlgcheck = (idx) => {
    dispatch(slice.actions.SHOW_DLG_CHECK({ visible: true, idx: idx }));
  };
    ...
    <div className={props.classes.root}>
      ...
      <AppBar position="static">
        <Toolbar>
          ...
          <Button  variant="contained" onClick={openDlgCopyPack}>复制包</Button>
