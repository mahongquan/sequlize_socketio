...
export default function todos(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case DELETE_TODO_RES:
      state = state.filter(todo => todo.id !== action.res.id);
      return state;      
    case EDIT_TODO:
    ...
    case COMPLETE_TODO_RES:
      console.log(action);
      state = state.map(todo =>
        todo.id === action.res.id ? { ...todo, completed:action.res.completed } : todo
      );
      console.log(state);
      return state;
    case COMPLETE_TODO:
