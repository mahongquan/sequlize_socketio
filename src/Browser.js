...
export default class Browser extends React.Component {
  ...
  buildMkdirUrl(path, name) {
    return '/fs/mkdir?path=' + path + '&name=' + name;
  }
  getParent(path, onSuccess) {
    this.props.socket.emit('/fs/parent', { path: path }, onSuccess);
  }

  loadFilesFromServer = path => {
    var self = this;
    this.props.socket.emit('/fs/children', { path: path }, data => {
  ...
  onParent = () => {
    ...
    this.getParent(this.currentPath(), onSuccess);
  };
  ...
  mkdir = () => {
    ...
    this.props.socket.emit(
      this.buildMkdirUrl(this.currentPath(), newFolderName),
      this.reloadFilesFromServer
    );
  };
  onClick = f => {
